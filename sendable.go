...
func (a *Audio) Send(b *Bot, to Recipient, opt *SendOptions) (*Message, error) {
	...
	if a.Thumbnail != nil {
		files = map[string]*File{"thumb": &a.Thumbnail.File}
	}

	msg, err := b.sendObject(&a.File, "audio", params, files)
	if err != nil {
		return nil, err
	}
...
func (d *Document) Send(b *Bot, to Recipient, opt *SendOptions) (*Message, error) {
	...
	var files map[string]*File
	if d.Thumbnail != nil {
		files = map[string]*File{"thumb": &d.Thumbnail.File}
	}

	msg, err := b.sendObject(&d.File, "document", params, files)
	if err != nil {
		return nil, err
	}
...
func (v *Video) Send(b *Bot, to Recipient, opt *SendOptions) (*Message, error) {
	...
	var files map[string]*File
	if v.Thumbnail != nil {
		files = map[string]*File{"thumb": &v.Thumbnail.File}
	}

	msg, err := b.sendObject(&v.File, "video", params, files)
	if err != nil {
		return nil, err
	}
...
func (v *VideoNote) Send(b *Bot, to Recipient, opt *SendOptions) (*Message, error) {
	...
	msg, err := b.sendObject(&v.File, "videoNote", params, files)
	if err != nil {
		return nil, err
	}
